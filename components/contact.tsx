...
export default function Contact() {
  ...
  const [logs, setLogs] = useState<LogLine[]>([
    ...
    { type: "system", text: "[* Connected to gateway: varunsinha.dev [AES-256-GCM]]" },
    ...
    if (cmd === "cat contact_info.json") {
      ...
      typeAndExecute(cmd, () => {
        setLogs((prev) => [
          ...
          {
            type: "output",
            text: "Connecting to linkedin.com/in/varunsinha20...\nConnection established.\nRedirecting to LinkedIn profile in background.",
          },
        ]);
        setTimeout(() => {
          window.open("https://linkedin.com/in/varunsinha20", "_blank");
        }, 800);
      });
      ...
      typeAndExecute(cmd, () => {
        setLogs((prev) => [
          ...
          {
            type: "output",
            text: "GitHub: github.com/VarunSinha07\nActive Repositories: PrakSat, Zit\nLoading GitHub overview in background.",
          },
        ]);
        setTimeout(() => {
          window.open("https://github.com/VarunSinha07", "_blank");
        }, 800);
      });
    } else if (cmd === "clear") {
      ...
      <div className="mx-auto w-full max-w-4xl px-4 md:px-6">
        ...
        <div className="w-full rounded-xl bg-zinc-950/90 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.8)] overflow-hidden font-mono text-xs sm:text-sm">
          ...
          <div className="px-4 py-3 bg-zinc-900 border-b border-white/10 flex items-center justify-between">
            ...
            <span className="text-zinc-500 text-xs">guest@varun.dev: ~</span>
              ...
              if (log.type === "command") {
                return (
                  <div key={index} className="text-white">
                    <span className="text-zinc-500">guest@varun.dev:~$ </span>
                    {log.text}
                  </div>
                );
              }
